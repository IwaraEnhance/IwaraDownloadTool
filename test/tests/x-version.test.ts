...
xVersionTestGroup.add(
    new Test('输出 40 位小写 hex', 'async', async function () {
...
xVersionTestGroup.add(
    new Test('相同输入结果稳定', 'async', async function () {
...
xVersionTestGroup.add(
    new Test('不同 expires 产生不同签名', 'async', async function () {
...
xVersionTestGroup.add(
    new Test('已知预期值（回归保护）', 'async', async function () {
