...
videoInfoGroup.add(new Test('isFullVideoInfo 完整有效对象', 'async', function () {
    this.assertTrue(isFullVideoInfo(makeFullVideoInfo()));
    this.assertTrue(isVideoInfo(makeFullVideoInfo()));
}));

videoInfoGroup.add(new Test('isFullVideoInfo 类型不匹配', 'async', function () {
    this.assertFalse(isFullVideoInfo(makeFullVideoInfo({ Type: 'partial' })));
}));

videoInfoGroup.add(new Test('isFullVideoInfo 缺少必需字段', 'async', function () {
...
videoInfoGroup.add(new Test('isFullVideoInfo 字段类型校验', 'async', function () {
...
videoInfoGroup.add(new Test('isFullVideoInfo 空 ID', 'async', function () {
...
videoInfoGroup.add(new Test('isPartialVideoInfo 有效对象', 'async', function () {
    this.assertTrue(isPartialVideoInfo(makePartialVideoInfo()));
    this.assertTrue(isVideoInfo(makePartialVideoInfo()));
}));

videoInfoGroup.add(new Test('isPartialVideoInfo 缺少可选字段仍有效', 'async', function () {
    // PartialVideoInfo 不要求 FileName/Size/DownloadUrl 等
    this.assertTrue(isPartialVideoInfo(makePartialVideoInfo()));
}));

videoInfoGroup.add(new Test('isPartialVideoInfo 类型不匹配', 'async', function () {
    this.assertFalse(isPartialVideoInfo(makePartialVideoInfo({ Type: 'full' })));
}));

videoInfoGroup.add(new Test('isPartialVideoInfo 必需字段校验', 'async', function () {
...
videoInfoGroup.add(new Test('isCacheVideoInfo 有效对象', 'async', function () {
    this.assertTrue(isCacheVideoInfo(makeCacheVideoInfo()));
    this.assertTrue(isVideoInfo(makeCacheVideoInfo()));
}));

videoInfoGroup.add(new Test('isCacheVideoInfo 缺少 RAW', 'async', function () {
    this.assertFalse(isCacheVideoInfo(makeCacheVideoInfo({ RAW: undefined })));
}));

videoInfoGroup.add(new Test('isCacheVideoInfo 类型不匹配', 'async', function () {
...
videoInfoGroup.add(new Test('isFailVideoInfo 有效对象', 'async', function () {
    this.assertTrue(isFailVideoInfo(makeFailVideoInfo()));
    this.assertTrue(isVideoInfo(makeFailVideoInfo()));
}));

videoInfoGroup.add(new Test('isFailVideoInfo 空 ID', 'async', function () {
    this.assertFalse(isFailVideoInfo(makeFailVideoInfo({ ID: '' })));
}));

videoInfoGroup.add(new Test('isFailVideoInfo 类型不匹配', 'async', function () {
    this.assertFalse(isFailVideoInfo(makeFailVideoInfo({ Type: 'full' })));
}));

// ============ isVideoInfo (综合) ============

videoInfoGroup.add(new Test('isVideoInfo 识别所有有效类型', 'async', function () {
    this.assertTrue(isVideoInfo(makeFullVideoInfo()));
    this.assertTrue(isVideoInfo(makePartialVideoInfo()));
    this.assertTrue(isVideoInfo(makeCacheVideoInfo()));
    this.assertTrue(isVideoInfo(makeFailVideoInfo()));
...
videoInfoGroup.add(new Test('isVideoInfo 拒绝无效对象', 'async', function () {
...
videoInfoGroup.add(new Test('assertVideoInfoType 返回正确类型', 'async', function () {
...
videoInfoGroup.add(new Test('assertVideoInfoType 未知类型抛错', 'async', function () {
