...
gmsyncTestGroup.add(
    new Test('构造读取初始值', 'async', function () {
        const d = new GMSyncDictionary<string>(
            'gmsync-t-construct',
            [
                ['a', 'va'],
                ['b', 'vb']
...
gmsyncTestGroup.add(
    new Test('set 触发 onSet 并可读回', 'async', function () {
...
gmsyncTestGroup.add(
    new Test('delete 触发 onDel', 'async', function () {
...
gmsyncTestGroup.add(
    new Test('delete 不存在的键不触发 onDel', 'async', function () {
...
gmsyncTestGroup.add(
    new Test('clear 触发 onSync 并清空', 'async', function () {
...
gmsyncTestGroup.add(
    new Test('validator 过滤非法初始值', 'async', function () {
        const d = new GMSyncDictionary<number>(
            'gmsync-t-valid',
            [
                ['a', 1],
                ['b', 'bad']
...
gmsyncTestGroup.add(
    new Test('GM 值变化监听器：注册/触发/移除', 'async', function () {
        GM_deleteValue('gm-listen-t1')
        const calls: any[] = []
        const id = GM_addValueChangeListener('gm-listen-t1', (name, oldValue, newValue, remote) => {
            calls.push({ name, oldValue, newValue, remote })
        })
...
gmsyncTestGroup.add(
    new Test('远程变化触发跨页同步', 'async', function () {
...
gmsyncTestGroup.add(
    new Test('本地 set 不触发远程处理（无自循环）', 'async', function () {
