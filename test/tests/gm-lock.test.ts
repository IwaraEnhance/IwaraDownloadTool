...
gmLockTestGroup.add(
    new Test('acquire 成功且 isHeld', 'async', function () {
...
gmLockTestGroup.add(
    new Test('他人持有时 acquire 失败', 'async', function () {
...
gmLockTestGroup.add(
    new Test('非持有者 renew 失败', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
        a.acquire('gm-test-lock', 10000)
...
gmLockTestGroup.add(
    new Test('租约过期后可被他人抢占', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('release 仅持有者可释放', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
        a.acquire('gm-test-lock', 10000)
...
gmLockTestGroup.add(
    new Test('pruneExpired 只清理过期锁', 'async', function () {
        const a = new GMLock('owner-A')
...
gmLockTestGroup.add(
    new Test('acquireWait 在他人释放后立即接管', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('acquireWait 在租约到期后接管', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('acquireWait 超时返回 false', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('极限: 远程覆盖写入后本地复核让位(最后写入者胜出)', 'async', function () {
        const a = new GMLock('owner-A')
...
gmLockTestGroup.add(
    new Test('极限: TTL=0 立即过期', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
        a.acquire('gm-ttl0', 0)
...
gmLockTestGroup.add(
    new Test('极限: 本页 acquire 等效续期并延长租约', 'async', async function () {
        const a = new GMLock('owner-A')
...
gmLockTestGroup.add(
    new Test('极限: 非持有者 renew 失败且不改变原租约', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('极限: 重复 release 幂等且不伤及新持有者', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('极限: 不同锁名互不干扰', 'async', function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('极限: acquireWait 多等待者接力, 唯一接管且逐个唤醒', 'async', async function () {
        ...
        for (let round = 0; round < waiters.length; round++) {
            ...
            const winnerIdx = await Promise.race(pendings.map((p, i) => p.then(() => i)).filter((_, i) => !resolved[i]))
            ...
            for (let i = 0; i < waiters.length; i++) {
                ...
                const state = await Promise.race([pendings[i].then(() => 'resolved' as const), new Promise<string>((r) => setTimeout(() => r('pending'), 80))])
...
gmLockTestGroup.add(
    new Test('极限: 持有者持续续期时等待者不误抢, 停止后到期接管', 'async', async function () {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const state = await Promise.race([pending.then(() => 'resolved' as const), new Promise<string>((r) => setTimeout(() => r('pending'), 50))])
...
gmLockTestGroup.add(
    new Test('极限: acquireWait timeout=0 立即失败', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('极限: 本页已持有时 acquireWait 立即返回', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
...
gmLockTestGroup.add(
    new Test('acquireWithHeartbeat 内部心跳自动续期', 'async', async function () {
        const a = new GMLock('owner-A')
...
gmLockTestGroup.add(
    new Test('失去锁后心跳停止并触发 onLost', 'async', async function () {
        const a = new GMLock('owner-A')
        let lost = false
        a.acquireWithHeartbeat('gm-hb-lost', 100, 20, () => {
            lost = true
        })
        // 模拟被另一页面覆盖(最后写入者胜出)
...
gmLockTestGroup.add(
    new Test('release 后心跳停止, 锁不被复活', 'async', async function () {
        const a = new GMLock('owner-A')
...
gmLockTestGroup.add(
    new Test('acquireWaitWithHeartbeat 接管后自动续期', 'async', async function () {
        const a = new GMLock('owner-A')
        const b = new GMLock('owner-B')
