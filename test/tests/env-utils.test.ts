...
utilsTestGroup.add(new Test('stringify undefined', 'async', function () {
    this.assertEqual(stringify(undefined), 'undefined');
}));

utilsTestGroup.add(new Test('stringify null', 'async', function () {
    this.assertEqual(stringify(null), 'null');
}));

utilsTestGroup.add(new Test('stringify boolean', 'async', function () {
    this.assertEqual(stringify(true), 'true');
    this.assertEqual(stringify(false), 'false');
}));

utilsTestGroup.add(new Test('stringify number', 'async', function () {
...
utilsTestGroup.add(new Test('stringify string', 'async', function () {
    this.assertEqual(stringify('hello'), 'hello');
    this.assertEqual(stringify(''), '');
}));

utilsTestGroup.add(new Test('stringify symbol', 'async', function () {
    const sym = Symbol('test');
    this.assertEqual(stringify(sym), sym.toString());
}));

utilsTestGroup.add(new Test('stringify function', 'async', function () {
    const fn = () => 42;
    this.assertEqual(stringify(fn), fn.toString());
}));

utilsTestGroup.add(new Test('stringify Error', 'async', function () {
    const err = new Error('test error');
    this.assertEqual(stringify(err), err.toString());
}));

utilsTestGroup.add(new Test('stringify Date', 'async', function () {
    const date = new Date('2023-01-01T00:00:00.000Z');
    this.assertEqual(stringify(date), '2023-01-01T00:00:00.000Z');
}));

utilsTestGroup.add(new Test('stringify object', 'async', function () {
...
utilsTestGroup.add(new Test('stringify array', 'async', function () {
...
utilsTestGroup.add(new Test('prune 移除 null 值', 'async', function () {
...
utilsTestGroup.add(new Test('prune 空对象', 'async', function () {
    const result = prune({});
    this.assertEqual(JSON.stringify(result), '{}');
}));

utilsTestGroup.add(new Test('prune 保留数字 0', 'async', function () {
...
utilsTestGroup.add(new Test('UUID 生成非空字符串', 'async', function () {
    const uuid = UUID();
    this.assertTrue(typeof uuid === 'string');
    this.assertTrue(uuid.length > 0);
}));

utilsTestGroup.add(new Test('UUID 格式 (32位十六进制)', 'async', function () {
    const uuid = UUID();
    // UUID 应为 32 位十六进制字符（crypto.randomUUID 去掉横线）
    this.assertTrue(/^[0-9a-f]{32}$/i.test(uuid));
}));

utilsTestGroup.add(new Test('UUID 唯一性', 'async', function () {
    const uuid1 = UUID();
    const uuid2 = UUID();
...
utilsTestGroup.add(new Test('delay 等待指定时间', 'async', async function () {
...
utilsTestGroup.add(new Test('throttle leading=true 首次立即执行', 'async', async function () {
    let callCount = 0;
...
utilsTestGroup.add(new Test('throttle 连续调用限频', 'async', async function () {
    let callCount = 0;
    const fn = throttle(() => { callCount++; }, 50);
    fn();                          // 立即执行 (leading)
    fn();                          // 被限流，安排 trailing
...
utilsTestGroup.add(new Test('throttle cancel 取消 trailing', 'async', async function () {
    let callCount = 0;
    const fn = throttle(() => { callCount++; }, 50);
    fn();                          // 立即执行
    fn();                          // 安排 trailing
...
utilsTestGroup.add(new Test('throttle leading=false 首次不执行', 'async', async function () {
    let callCount = 0;
    const fn = throttle(() => { callCount++; }, 30, { leading: false, trailing: true });
    fn();
...
utilsTestGroup.add(new Test('throttle 传参正确', 'async', async function () {
    let result = '';
    const fn = throttle((s: string) => { result = s; }, 30);
    fn('hello');
...
utilsTestGroup.add(new Test('debounce 延迟执行', 'async', async function () {
    let callCount = 0;
...
utilsTestGroup.add(new Test('debounce 连续调用只执行最后一次', 'async', async function () {
    let result = '';
    const fn = debounce((s: string) => { result = s; }, 30);
    fn('A');
    fn('B');
    fn('C');
...
utilsTestGroup.add(new Test('debounce cancel 取消执行', 'async', async function () {
    let callCount = 0;
    const fn = debounce(() => { callCount++; }, 30);
    fn();
    fn.cancel();
...
utilsTestGroup.add(new Test('debounce immediate=true 首次立即执行', 'async', async function () {
    let callCount = 0;
    const fn = debounce(() => { callCount++; }, 50, { immediate: true });
    fn();
...
utilsTestGroup.add(new Test('debounce immediate 连续调用', 'async', async function () {
    let callCount = 0;
    const fn = debounce(() => { callCount++; }, 50, { immediate: true });
    fn(); // 立即执行
    fn(); // 被防抖
