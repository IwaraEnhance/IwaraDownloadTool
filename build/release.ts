 ...
 *
 * 职责划分:
 * - 本地（本脚本）: 在 dev 分支上 同步远程 → 提升版本号 → 验证构建 → 提交 → 打标签 → 推送 dev + vX.Y.Z 标签
 * - CI（.github/workflows）: 仅当推送 vX.Y.Z 版本标签时构建发布
 *     - 标签指向 dev    → Preview.yml 构建 preview 渠道 → 发布/移动 preview release
 *     - 标签指向 master → Release.yml 构建 latest 渠道 → 创建 vX.Y.Z 存档 + latest release
 *                    ⚠ 之后会把远程 dev 删除并重建到 master（远程分支变化）
 *
 * 版本标签由本地创建并随 dev 一起推送，作为 CI 发布触发器（分支推送不再触发发布）。
 * 合并 dev → master 后，需将同一标签 vX.Y.Z 重新指向 master 合并提交并推送，触发正式版发布:
...
function main(): void {
    ...
    try {
        ...
        // 打版本标签（作为 CI 发布触发器）；标签已存在则中止以触发回滚
        if (!options.dryRun && exec(`git tag -l v${newVersion}`)) {
            throw new Error(`标签 v${newVersion} 已存在，疑似重复发布。若确需重建请先手动删除该标签。`)
        }
        log(TAG, `创建版本标签 v${newVersion}...`)
        run(`git tag v${newVersion}`, { tag: TAG, dryRun: options.dryRun })
    } catch (err) {
        error(TAG, `发布失败: ${err}`)
        if (!options.dryRun) {
            ...
            if (head === exec(`git rev-parse ${backupCommit}`)) {
                ...
                try {
                    ...
                    // 删除发布流程已创建的版本标签（若已创建）
                    try {
                        run(`git tag -d v${newVersion}`, { tag: TAG, silent: true })
                    } catch {
                        // 标签未创建，无需删除
                    }
    ...
    // ── 6. 推送 dev 与版本标签（标签推送触发发布工作流；推送后失败不回滚，避免本地/远程分叉） ──
    log(TAG, `推送 ${branch} 分支及版本标签 v${newVersion}（标签推送触发发布工作流）...`)
    try {
        run(`git push origin ${branch}`, { tag: TAG, dryRun: options.dryRun })
        run(`git push origin v${newVersion}`, { tag: TAG, dryRun: options.dryRun })
    } catch (err) {
        error(TAG, `推送失败: ${err}`)
        warn(TAG, '本地提交/标签已创建但未完全推送，常见原因: 远程 dev 已被 CI 重建（落后/分叉）')
        warn(TAG, `解决: git fetch origin && git rebase origin/dev 后手动执行 git push origin dev && git push origin v${newVersion}`)
        process.exit(1)
    }

    success(TAG, `已发布 v${newVersion}（版本提交与标签已推送到 ${branch}）`)
    log(TAG, 'CI 将根据标签指向构建发布: 标签指向 dev → preview 预览版')
    log(TAG, `后续步骤: 合并 dev → master，并将标签 v${newVersion} 重新指向 master 合并提交后推送，触发 latest 正式版发布`)
}
